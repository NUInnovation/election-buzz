...
class Tweet(object):
	...
	def __init__(self,id=0,text='',words='', stems='', username='xx',tweet='',categories=[]):
		self.tweet = tweet 
		self.id = id
		self.text = text
		self.words = words
...
def get_best_category(tweet, clusters):
	""" gets the best possible category based on the clusters of tweets"""


	distances = {}
	for key in clusters:
		distances[key] = dot_product(tweet.vector.vector, clusters[key].centroid)

	best_cat = max(distances, key=distances.get)
